...
во время сборки образа создаётся непривелегированный пользователь по умолчанию
...
`pyproject.toml` из корневого каталога репозитория.
1. Установка зависимостей из `poetry.lock` без модуля с исходниками проекта,
так как остальные файла из каталога проекта.
...
Базовый образ получает тег `comp_ds_prj_base`.
...
1. Поиск и загрузка ранее собранного базового образа `comp_ds_prj_base` в
   локальном регистре.
1. Переключение на учётныую запись превилегированного пользователя `root`.
1. Копирование корневого каталога репозитория.
1. Рекурсивная смена владельца скопированного каталога.
1. Переключение на превилегированного пользователя (по умолчанию `dockeruser`).
1. Установка рабочим каталогом `/dockeruser/comp_ds_prj`.
1. Установка недостающих модулей с помощью Poetry - установка `comp_ds_prj`.
...
| UID      | ID пользователя, указанного в UNAME.                                                                  | 1001                           |
| GID      | ID группы пользователя, указанного в UNAME.                                                           | 1001                           |
| REPO     | Имя удалённого репозитория GitHub в формате `имя_пользователя/название_репозитория.git` | AlekseiBogachev/competitive_ds_cource_prj.git |

Образ локальной среды разработки получает тег `comp_ds_prj_dev`.
...
- `REPO="AlekseiBogachev/competitive_ds_cource_prj.git"` - имя удалённого
